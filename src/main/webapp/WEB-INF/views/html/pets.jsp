...
.no-edit {
    pointer-events: none;
    background-color: #f8f9fa; /* Màu nền xám nhạt để chỉnh sửa không được phép */
}
</style>
</head>
									...
									<table class="table">
										<thead>
											<tr class="text-center">
												<th><a
													href="?page=${currentPage}&size=${size}&sortBy=maThuCung&sortDir=${sortBy == 'maThuCung' ? (sortDir == 'asc' ? 'desc' : 'asc') : 'asc'}">Mã</a></th>
												<th><a
													href="?page=${currentPage}&size=${size}&sortBy=tenThuCung&sortDir=${sortBy == 'tenThuCung' ? (sortDir == 'asc' ? 'desc' : 'asc') : 'asc'}">Tên</a></th>
												<th><a
													href="?page=${currentPage}&size=${size}&sortBy=loai&sortDir=${sortBy == 'loai' ? (sortDir == 'asc' ? 'desc' : 'asc') : 'asc'}">Loài</a></th>
												<th><a
													href="?page=${currentPage}&size=${size}&sortBy=giong&sortDir=${sortBy == 'giong' ? (sortDir == 'asc' ? 'desc' : 'asc') : 'asc'}">Giống</a></th>
												<th><a
													href="?page=${currentPage}&size=${size}&sortBy=tuoi&sortDir=${sortBy == 'tuoi' ? (sortDir == 'asc' ? 'desc' : 'asc') : 'asc'}">Tuổi</a></th>
												<th><a
													href="?page=${currentPage}&size=${size}&sortBy=gia&sortDir=${sortBy == 'gia' ? (sortDir == 'asc' ? 'desc' : 'asc') : 'asc'}">Giá</a></th>
												<th>Hình ảnh</th>
												<th>Thao tác</th>
											</tr>
										</thead>
											...
											<c:forEach var="item" items="${items}">
												<tr class="text-center">
													...
													<td><fmt:formatNumber value="${item.gia}"
															currencyCode="USD" currencySymbol="$" type="currency"></fmt:formatNumber></td>
													<td><img class="w-25" id="${item.hinhAnh}"
														alt="${item.hinhAnh}"
														src="${pageContext.request.contextPath}/hinhAnh/pets/${item.hinhAnh}" /></td>
													<td class="text-center">
														<div class="btn-group">
															...
															<ul class="dropdown-menu">
																<li><a class="dropdown-item edit-link"
																	href="/admin/pets1/edit/${item.maThuCung}"
																	data-id="${item.maThuCung}"
																	data-name="${item.tenThuCung}" data-loai="${item.loai}"
																	data-giong="${item.giong}" data-tuoi="${item.tuoi}"
																	data-gia="${item.gia}" data-hinhanh="${item.hinhAnh}"
																	data-bs-toggle="modal" data-bs-target="#editPetModal">Edit</a></li>
																<li><a class="dropdown-item"
																	href="${pageContext.request.contextPath}/admin/pets1/delete/${item.maThuCung}">Delete</a></li>
															</ul>
														</div>
													</td>
												</tr>
										...
										<ul class="pagination">
											<c:if test="${currentPage > 0}">
												<li class="page-item"><a class="page-link"
													href="?page=${currentPage - 1}&size=${size}&sortBy=${sortBy}&sortDir=${sortDir}">Previous</a></li>
											</c:if>
											<c:forEach var="i" begin="0" end="${totalPages - 1}">
												<li
													class="page-item <c:if test="${i == currentPage}">active</c:if>">
													<a class="page-link"
													href="?page=${i}&size=${size}&sortBy=${sortBy}&sortDir=${sortDir}">${i + 1}</a>
												</li>
											</c:forEach>
											<c:if test="${currentPage < totalPages - 1}">
												<li class="page-item"><a class="page-link"
													href="?page=${currentPage + 1}&size=${size}&sortBy=${sortBy}&sortDir=${sortDir}">Next</a></li>
											</c:if>
										</ul>
									</nav>
								</div>
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												<h5 class="modal-title" id="editPetModalLabel">Chỉnh
													sửa thông tin thú cưng</h5>
												<button type="button" class="btn-close"
													data-bs-dismiss="modal" aria-label="Close"></button>
											</div>
											<div class="modal-body">
												<form:form action="/admin/pets1/save" modelAttribute="item"
													enctype="multipart/form-data">
													<div class="mb-3">
														<form:hidden path="maThuCung" id="maThuCung"
															class="form-control" />
													</div>
													<div class="mb-3">
														<label for="tenThuCung" class="form-label">Tên thú
															cưng</label>
														<form:input required="required" path="tenThuCung"
															id="tenThuCung" class="form-control" />
													</div>
													<div class="mb-3">
														<label for="loai" class="form-label">Loại</label>
														<form:input required="required" path="loai" id="loai"
															class="form-control" />
													</div>
													<div class="mb-3">
														<label for="giong" class="form-label">Giống</label>
														<form:input required="required" path="giong" id="giong"
															class="form-control" />
													</div>
													<div class="mb-3">
														<label for="tuoi" class="form-label">Tuổi</label>
														<form:input required="required" path="tuoi" id="tuoi"
															type="number" class="form-control" />
													</div>
													<div class="mb-3">
														<label for="gia" class="form-label">Giá</label>
														<form:input required="required" path="gia" id="gia"
															type="number" class="form-control" />
													</div>
													<div class="mb-3">
														<label for="file" class="form-label">Hình ảnh</label>
														<div class="input-group">
															<!-- Input field for displaying selected file name -->
															<form:input type="text" path="hinhAnh" id="hinhAnh"
																class="form-control no-edit" />
															<!-- Custom file input button -->
															<input type="file" class="form-control" id="file"
																name="saveFile" style="display: none;" /> <label
																for="file" class="input-group-text"> <i
																class="fa-solid fa-file"></i> <!-- Bootstrap icon, change to your preferred icon -->
															</label>
														</div>
													</div>
													<div class="mb-3">
														<button formaction="/admin/pets1/save"
															class="btn btn-primary">Lưu</button>
														<button formaction="/admin/pets1/update"
															class="btn btn-primary">Sửa</button>
														<button formaction="/admin/pets1/delete/${item.maThuCung}"
															class="btn btn-danger">Xóa</button>
														<a href="/admin/pets1/index" class="btn btn-secondary">Trở
															lại</a>
													</div>
												</form:form>
											</div>
	...
	<script>
		$(document).ready(function() {
			$('.edit-link').click(function(event) {
				...
				// Lấy dữ liệu từ các thuộc tính data của thẻ <a>
				var maThuCung = $(this).data('id');
				var tenThuCung = $(this).data('name');
				var loai = $(this).data('loai');
				var giong = $(this).data('giong');
				var tuoi = $(this).data('tuoi');
		...
		// Lắng nghe sự kiện change của input file button
		document.getElementById("file").addEventListener("change", function() {
			// Lấy tên của tệp được chọn
			var fileName = this.files[0].name;
			// Gán tên của tệp vào trường input
